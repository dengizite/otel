...
io.on('connection', (socket) => {
	...
	socket.on('get_data',(data)=>{console.log(data)
		...
		else if(data[0]==='rooms_data'){io.to(socket.id).emit('get_data',['rooms_data',rooms])
			//for(let i in rooms){}
		}
	...
	socket.on('send_data',(data)=>{console.log(data)
		...
		else if(data[0]==='add_room'){rooms[data[1]]={price:data[2],bad:data[3],cat:data[4],descr:data[5],stat:'свободен'};console.log(53,rooms)}
	})
